...
export default function ContractPrintPage() {
  ...
  useEffect(() => {
    ...
    async function loadContractData() {
      ...
      if (itemsRes.data && itemsRes.data.length > 0) {
        ...
        setLineItems([{ skip_size: conRes.data.size || "6m³", quantity: qty, unit_price: conRes.data.mrr / qty }]);
      }
  ...
  // MATH LOGIC: Base MRR is the Subtotal. Add VAT on top.
  ...
  if (!contract) return <div className="p-8 text-center font-sans"><h2 className="text-xl font-bold">Contract not found</h2><button onClick={() => router.back()} className="mt-4 text-cyan-600 underline">Return to Ledger</button></div>;
  ...
  return (
    <div className="min-h-screen bg-zinc-100 py-8 font-sans text-zinc-900 print:bg-white print:py-0">
      ...
      <div className="mx-auto max-w-[800px] bg-white p-12 shadow-xl print:m-0 print:max-w-none print:shadow-none sm:rounded-lg">
        ...
        {/* Header (Company info pulled from settings) */}
        <div className="mb-12 flex items-start justify-between">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-tighter text-zinc-900">{company.company_name || "SkipCommand"}</h1>
            <div className="mt-3 text-sm leading-relaxed text-zinc-600">
              {company.reg_number && <p>Reg: {company.reg_number}</p>}
              <p>VAT: {company.vat_number || "Not Registered"}</p>
              {company.address && <p className="whitespace-pre-wrap">{company.address}</p>}
              <p className="mt-1">{company.phone && <span className="mr-3">Tel: {company.phone}</span>}{company.email && <span>Email: {company.email}</span>}</p>
            </div>
          </div>
        ...
        <div className="mb-10 grid grid-cols-2 gap-10">
          <div><h3 className="mb-2 text-xs font-bold uppercase tracking-widest text-zinc-400">Client</h3><p className="text-lg font-bold">{contract.client}</p></div>
        ...
        <div className="flex justify-end">
          <div className="w-1/2 min-w-[250px]">
            <div className="flex justify-between border-b border-zinc-100 py-2 text-sm text-zinc-600"><span>Subtotal (Excl. VAT)</span><span className="font-mono">{zar.format(subtotal)}</span></div>
            {isVatRegistered && (<div className="flex justify-between border-b border-zinc-200 py-2 text-sm text-zinc-600"><span>VAT (15%)</span><span className="font-mono">{zar.format(vatAmount)}</span></div>)}
            <div className="flex justify-between py-3 text-lg font-black text-zinc-900"><span>Monthly MRR</span><span className="font-mono">{zar.format(totalDue)}</span></div>
          </div>
        </div>
      </div>
    </div>
