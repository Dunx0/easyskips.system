...
import {
  Truck, FileSignature, Container, CalendarDays, User, MapPin,
  CreditCard, Loader2, CheckCircle2, XCircle, Send, Hash, Gauge, X,
  Scale, Plus, Trash2,
} from "lucide-react";

/* fallback rate card — used ONLY if the rates table is empty */
...
function parseQty(items) {
  if (!items) return 0;
  let total = 0; const re = /(\d+)\s*[x×]/g; let m;
      ...
      {options.map((opt) => (
        <button key={opt} type="button" onClick={() => onChange(opt)}
          className={`flex-1 rounded-lg px-2 py-1.5 text-xs font-semibold transition-all duration-200 ${
            value === opt ? "bg-gradient-to-br from-amber-400 to-orange-500 text-[#0F0F13]" : s.segOff}`}>
          {opt}
        </button>
      ))}
    </div>
...
function LineItems({ s, lines, setLines, hire, priceMode, rates }) {
  ...
  const changeSize = (idx, size) =>
    setLines((prev) => prev.map((l, i) => (i === idx ? { ...l, size, price: defaultPrice(size) } : l)));
  const addLine = () => {
    const used = new Set(lines.map((l) => l.size));
    const next = rates.find((x) => !used.has(x.size))?.size ?? rates[0]?.size ?? "6m³";
    ...
    <div className="space-y-2">
      ...
      {lines.map((line, idx) => (
        <div key={idx} className="grid grid-cols-[1fr_64px_104px_92px_32px] items-center gap-2">
          ...
          <button type="button" onClick={() => removeLine(idx)} disabled={lines.length === 1} aria-label="Remove line"
...
export default function DispatchPage() {
  ...
  const [rates, setRates] = useState(FALLBACK_RATES);  // live rate card
  const rateFor = useCallback((size, hire) => {
    const r = rates.find((x) => x.size === size);
    return r ? Number(r[hire.toLowerCase()]) || 0 : 0;
  }, [rates]);

  const [delivery, setDelivery] = useState(EMPTY_DELIVERY);
  const [invLines, setInvLines] = useState([{ size: "6m³", qty: 1, price: 0 }]);
  const [contractForm, setContractForm] = useState(EMPTY_CONTRACT);
  const [contractLines, setContractLines] = useState([{ size: "6m³", qty: 1, price: 0 }]);
  ...
  const loadContext = useCallback(async () => {
    const [rt, c, e, n] = await Promise.all([
    ...
    const liveRates = (rt.data && rt.data.length) ? rt.data : FALLBACK_RATES;
    setRates(liveRates);
    setContracts(c.data ?? []);
  ...
  const setDeliveryField = (key, value) => setDelivery((prev) => ({ ...prev, [key]: value }));
  ...
  const invoiceAmount = useMemo(() => invLines.reduce((sum, l) => sum + l.qty * l.price, 0), [invLines]);
  const invoiceSkips = useMemo(() => invLines.reduce((sum, l) => sum + l.qty, 0), [invLines]);
  const contractMrr = useMemo(() => contractLines.reduce((sum, l) => sum + l.qty * l.price, 0), [contractLines]);
  const contractSkips = useMemo(() => contractLines.reduce((sum, l) => sum + l.qty, 0), [contractLines]);
  ...
  const budget = useMemo(() => {
    ...
    return { covered: matchedContract.skips, extrasOut, afterThis: matchedContract.skips + extrasOut + invoiceSkips };
  ...
  async function handleDeliverySubmit(e) {
    ...
    try {
      const row = {
        id, client: delivery.client.trim(), date: delivery.date,
        items: buildItemsString(invLines), amount: invoiceAmount,
        banked: false, collected: false, hire: delivery.hire, payment: delivery.payment,
        vehicle: delivery.vehicle, driver: delivery.driver,
        location: delivery.location.trim() || null, contract_id: matchedContract?.id ?? null,
      };
  ...
  async function handleContractSubmit(e) {
    ...
    try {
      ...
      const row = {
        client: contractForm.client.trim(), site: contractForm.site.trim(),
        skips: contractSkips, size: primarySize, mrr: contractMrr, since: today(),
      };
      ...
      <div className="mx-auto max-w-[1200px]">
        <header className="mb-7">
          ...
          <p className={`mt-1 flex items-center gap-1.5 text-xs ${s.sub}`}>
            <CalendarDays size={12} /> Log deliveries and register contract clients — rates pulled live from Settings
          </p>
        </header>
          ...
          <section className={`rounded-2xl p-6 ${s.panel}`}>
            ...
            <form onSubmit={handleDeliverySubmit} className="space-y-4">
              ...
              {matchedContract && budget && (
                <div className="rounded-xl border border-amber-400/40 bg-amber-500/[0.08] p-4">
                  ...
                  <p className={`mt-1.5 text-xs leading-relaxed ${s.sub}`}>
                    ...
                    {budget.extrasOut > 0 && <> They already have <span className="font-bold tabular-nums text-amber-400">{budget.extrasOut} extra</span> on site.</>}{" "}
                  ...
                  <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
                    <div className={`rounded-lg px-3 py-2 text-[11px] leading-snug ${s.chip}`}>
                      <span className="font-bold text-amber-400">Option A (this form):</span> submit below — billed as a separate ad-hoc invoice, linked to the contract.
                    </div>
              ...
              <Field s={s} label="Skips" icon={Container} hint="add as many sizes as needed">
                <LineItems s={s} lines={invLines} setLines={setInvLines} hire={delivery.hire} priceMode="hire" rates={rates} />
              </Field>
              ...
              <SubmitButton busy={submittingInvoice}
                label={matchedContract ? "Log as contract extra" : "Dispatch & Log"} busyLabel="Logging…" icon={Send} />
            </form>
          </section>
          ...
          <section className={`h-fit rounded-2xl p-6 ${s.panel}`}>
            ...
            <form onSubmit={handleContractSubmit} className="space-y-4">
              ...
              <Field s={s} label="Committed skips" icon={Hash} hint="monthly recurring fleet">
                <LineItems s={s} lines={contractLines} setLines={setContractLines} hire="Monthly" priceMode="monthly" rates={rates} />
              </Field>
